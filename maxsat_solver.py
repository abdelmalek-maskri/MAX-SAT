...
def parse_wdimacs_file(wdimacs_file: str):
    ...
    Parses a WDIMACS file and returns:
      - num_vars: number of variables (from the problem line)
      - num_clauses: number of clauses (from the problem line)
      - clauses: a list of clauses, where each clause is represented as a list of integers.
                 The first value in each clause line is ignored, and the trailing 0 is dropped.
                 
    Example:
        For a clause line: "0 1 -2 3 0"
        the returned clause will be: [1, -2, 3]
    """
    num_vars = 0
    num_clauses = 0
    clauses = []
    ...
    try:
        with open(wdimacs_file, 'r') as f:
            for line in f:
                ...
                if len(parts) < 2:
                    continue
                # According to the assignment, ignore the first element and the trailing 0.
...
def count_satisfied_clauses_from_parsed(assignment: str, clauses: list) -> int:
    ...
    The assignment is provided as a bitstring.
    """
    satisfied_count = 0
